...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="dc" select="document(concat('http://localhost:8080/oaitransform/transformed?pid=', $pid,'&amp;action=dc'))" />
	<metadata>
        <rdf:RDF>
            <edm:ProvidedCHO>
		...
		<xsl:variable name="creators" select="document(concat('http://localhost:8080/oaitransform/transformed?pid=', $pid,'&amp;action=dc'))/oai_dc:dc/dc:creator"/>
		<xsl:variable name="amount" select="count($creators)" />
		
		<xsl:choose>
			<xsl:when test="$amount != 0">
				<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=', $pid,'&amp;action=dc'))/oai_dc:dc/*[following-sibling::dc:creator[$amount]]" />
				<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=',     $pid,'&amp;action=agent'))/edm:record/dc:creators/*"/>
				<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=', $pid,'&amp;action=dc'))/oai_dc:dc/*[preceding-sibling::dc:creator[$amount]]" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=',     $pid,'&amp;action=agent'))/edm:record/dc:creators/*"/>
				<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=', $pid,'&amp;action=dc'))/oai_dc:dc/*" />
			</xsl:otherwise>
		</xsl:choose>
        ...
        <edm:WebResource>
		<xsl:attribute name="rdf:about"><xsl:value-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=',     $pid,'&amp;action=europeana'))/edm:record/edm:object/@rdf:resource"/></xsl:attribute>
        </edm:WebResource>
	  ...
	  <!-- isShownAt, isShownBy, rights, object -->
          <xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=',     $pid,'&amp;action=europeana'))/edm:record/*"/>

          <edm:provider>Czech digital library/Česká digitální knihovna</edm:provider>

         </ore:Aggregation>
	 
	<xsl:copy-of select="document(concat('http://localhost:8080/oaitransform/transformed?pid=',     $pid,'&amp;action=agent'))/edm:record/edm:agents/*"/>
